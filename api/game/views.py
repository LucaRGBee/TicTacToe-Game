...
from .serializers import UserSerializer, GameSerializer, PasswordSerializer, JoinSerializer, TurnSerializer, ResponseSerializer, NextSerializer
...
class CreateGameView(APIView):
    ...
    @extend_schema(
        request=PasswordSerializer,  # <-- tell Swagger the request body
...
class CreateNextGameView(APIView):
    ...
    @extend_schema(
        request=NextSerializer,  # <-- tell Swagger the request body
        responses={201: GameSerializer},
        description="Create a new game. Optionally provide a password."
    )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if pre.playerOne != self.request.user and pre.playerTwo != self.request.user:
            return Response({"error": "you must be part of a game"})
        if not pre.ended:
            return Response({"error": "you cannot start upon a running game"})
        
        if pre.next:
            return Response({"error": "this game already has a next one started"})
...
class GetGamesUserView(generics.ListAPIView):
    ...
    def get_queryset(self):
        games1 = Game.objects.filter(playerOne=self.kwargs["id"])
        games2 = Game.objects.filter(playerTwo=self.kwargs["id"])
...
class JoinGameView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if game.ended:
            return Response({"error": "this game has ended use /game/get/<id> instead"})
        ...
        if p == game.playerOne:
            return Response({"error": "cannot join the same game twice"})
...
class MakeTurnView(APIView):
    ...
    def checkGame(self, board, turn):
        for y in range(3):
            if abs(sum(board[y])) == 3 or abs(sum(board[x][y] for x in range(3))) == 3 or abs(sum(board[i][i] for i in range(3))) == 3 or abs(sum(board[i][2-i] for i in range(3))) == 3:
                return 1
    ...
    def post(self, request, *args, **kwargs):
        ...
        if game.board[y][x] != 0:
            return Response({"error": "the position you chose was already taken"}, status=400)
